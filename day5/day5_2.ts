...
const unsortedPages = pages.filter(
  (page) =>
    JSON.stringify(page) !==
    JSON.stringify([...page].sort((a, b) => sortMap.get(b)?.get(a) ?? 0)),
);

const newlySortedPages = unsortedPages.map((page) =>
  [...page].sort((a, b) => sortMap.get(b)?.get(a) ?? 0),
);

const sumOfMiddleNumbers = newlySortedPages.reduce(
